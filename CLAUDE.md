...
**주요 라이브러리**: `package.json` 의 `dependencies` 참조. 사용 정책상 특기할 항목은 본 문서 §6 (Zustand), §8 (Firebase), §9 (WebView/차트), §12 (의존성 관리) 에서 규칙으로 정의한다. `@react-native-async-storage/async-storage` 는 식별자 저장 전용 (§6.4 예외).

**엔트리포인트**: `index.js` → `App.tsx` → `AppNavigator` (4탭: 홈/차트/거래/설정)

**폴더 구조**: `src/{screens,components,services,store,utils,types,navigation}/` (§10.1 에서 역할 서술)
...
앱의 비즈니스 로직 구현 시 준수해야 하는 원칙. (데이터 불변성 / 명시적 검증 같은 일반 원칙은 §4 TypeScript 규칙과 §6 Zustand 규칙에 이미 녹아있으므로 본 섹션에서는 중복 기술하지 않는다.)
...
앱 크래시가 사용자 경험을 해치므로 Python 의 `raise RuntimeError` 를 1:1 이식하지 않는다. §11.2 (Error Boundary 없음) 와 §19.4 (사용자 중심) 를 함께 고려하여, 다음 3가지 중 문맥에 맞는 방식을 선택한다:
...
**3) 치명적 경로만 throw** — 복구 불가능한 내부 모순(서버 계약 위반 등) 은 `throw new Error('내부 불변조건 위반: ...')` 로 즉시 중단. RN 기본 처리에 맡긴다(§11.2).

**구분 기준**:

- 입력 파라미터 검증 (외부에서 잘못된 값 전달 가능) → §11.4 사용자 표시 에러 규칙 적용
...
- `device_id` 는 설치 UUID. `AsyncStorage` 에 영구화(§6.4 예외). 근거 위치: `src/services/fcm.ts::getDeviceId`, `src/services/fcm.ts::requestNotificationPermission`
...
## 9. WebView + TradingView Charts 규칙
...
- **사용자에게 표시되는 에러는 §11.4 규칙 우선** — `console.error` 는 개발자 로그용, 사용자 메시지는 별도 한글 텍스트
...
> 내부 불변조건(로직상 도달 불가) 처리는 §3.1 불가능 조건 처리 를 따른다.
...
1. Claude Code 는 새 의존성 추가 시 **반드시 사용자에게 질문** (§2.2 형식)
...
| AsyncStorage / SQLite 로 RTDB 데이터·세션 캐싱 | 메모리 캐시만 (예외: 설치 UUID 1개, §6.4) |
