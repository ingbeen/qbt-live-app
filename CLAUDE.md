...
**주요 라이브러리** (package.json 기준):

- 상태: `zustand`
- 네비게이션: `@react-navigation/native`, `@react-navigation/bottom-tabs`
- Firebase: `@react-native-firebase/{app,auth,database,messaging}` (v24)
...
- **이모지 금지**: `😀👍❌✅` 등. 허용 기호 화이트리스트는 `src/utils/constants.ts::SYMBOLS` (⚠ ⚡ ▲ ▼ ● ✕ →) 를 사용한다.
...
- 앱이 쓸 수 있는 경로는 **5개만** (근거 위치: `src/utils/constants.ts::RTDB_PATHS`):
  - `/fills/inbox/{uuid}`
...
- **날짜**: moment.js, date-fns (내장 Date + 유틸 함수 사용. 실제 필요 발생 시 별도 검토)
...
- **단위 테스트 필수 아님**. 사용자 검증 중심 (의미 단위로 에뮬레이터/기기 확인).
- 기능 단위 작업 완료마다 사용자가 실제 동작을 확인.

### 14.2 추후 추가 시 (MVP 범위 밖, 필요 발생 시 검토)
