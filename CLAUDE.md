...
상세 아키텍처는 `docs/DESIGN_APP.md §3`, 데이터 계약은 `docs/DESIGN_QBT_LIVE_FINAL.md §8.2`, 기술 스택 상세는 `docs/DESIGN_APP.md §1` 을 SoT 로 참조.
...
- **RTDB 경로 트리와 동일한 구조** (`docs/DESIGN_APP.md §6.2` 참고)
- **탭별 분리 금지**. 데이터별 캐시.
...
- **전문 용어** (Model, Actual, drift 등): 영문 그대로 사용 (§`docs/DESIGN_APP.md §17`)
- **이모지 금지**: `😀👍❌✅` 등. 허용 기호는 §`docs/DESIGN_APP.md §17.4` 참고.
...
- 앱이 쓸 수 있는 경로는 **5개만** (§`docs/DESIGN_APP.md §5.2`):
  - `/fills/inbox/{uuid}`
...
- 메시지 타입 **화이트리스트** (§`docs/DESIGN_APP.md §14.4`):
...
- Firebase RTDB Rules 는 서버에서 설정 (§`docs/DESIGN_APP.md §19`)
- 앱은 Rules 변경 안 함
- OWNER_UID 는 환경변수/상수로 **참고용만** 유지 (`docs/DESIGN_APP.md §0.1`)
...
- `android/app/build.gradle`:
  - `minSdkVersion 24`
  - `targetSdkVersion 35` (Android 15, RN 0.85 템플릿 기본)
...
1. 사용자의 명시적 지시 (최우선)
2. 본 문서 `CLAUDE.md`
3. `docs/DESIGN_APP.md`
4. `docs/DESIGN_QBT_LIVE_FINAL.md` (서버 계약, 단 데이터 계약 영역만)
5. 라이브러리 공식 문서
6. 일반적 모범 사례
