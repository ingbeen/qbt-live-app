...
> **추가 학습 규칙**: 작업 대상 폴더에 `CLAUDE.md` 가 존재하면 본 문서와 함께 추가 학습한다. 외부 라이브러리/도구 최신 사용법은 Context7 우선, 부족 시 웹검색으로 보완 (§4.3).
> **관련 문서**:
>
> - `docs/DESIGN_QBT_LIVE_FINAL.md` — 서버↔앱 RTDB 데이터 계약 (서버 SoT, 변경 금지)
> - `docs/COMMANDS.md` — 프로젝트 관련 **모든 명령어** 단일 관리처
> - `docs/CLAUDE.md` — docs 폴더 운영 규칙 / 계획서(Plan) 운영 SoT
...
**주요 라이브러리**: `package.json` 의 `dependencies` 참조. 사용 정책상 특기할 항목은 본 문서 §8 (Zustand), §10 (Firebase), §11 (WebView/차트), §14 (의존성 관리) 에서 규칙으로 정의한다. `@react-native-async-storage/async-storage` 는 식별자 저장 전용 (§8.4 예외).

**엔트리포인트**: `index.js` → `App.tsx` → `AppNavigator` (4탭: 홈/차트/거래/설정)

**폴더 구조**: `src/{screens,components,services,store,utils,types,navigation}/` (§12.1 에서 역할 서술)

데이터 계약(서버 SoT)은 `docs/DESIGN_QBT_LIVE_FINAL.md §8.2` 를 참조. 그 외 앱 아키텍처/구현 규칙은 본 문서가 단일 SoT.

---

## 1. 개발 원칙 (모든 작업의 기본 태도)
...
> **자가 검증**: "시니어 엔지니어가 보면 과설계라고 할까?" 가 yes 면 단순화한다. 200 줄로 쓴 것이 50 줄로도 가능하면 다시 쓴다.
...
- 본인이 다르게 썼을 스타일이라도 **기존 스타일에 맞춘다** (일관성 우선).
...
**검증 기준**: 변경된 모든 라인이 사용자 요청과 직접 연결되는가? — yes 가 아니면 그 라인은 빼라.

```typescript
// 사용자 요청: "formatUSD 의 소수점 자리수를 2 → 3 자리로 바꿔줘"

// ✗ 금지 — 요청 외 변경 (변수명 개선 / 주석 추가 / 인접 함수 리팩토링)
export const formatUSD = (amount: number): string => {
  // 미국 달러 포맷 (천 단위 콤마 + 소수점 3자리)
...
export const formatUSD = (amount: number): string =>
  `$${amount.toLocaleString('en-US', {
    minimumFractionDigits: 3, // ← 변경된 라인
    maximumFractionDigits: 3, // ← 변경된 라인
  })}`;
```
...
**원칙**: 모든 코드 변경은 계획서를 작성한 후 진행한다.

### 3.1 예외 (계획서 없이 바로 진행 가능)

- 오타 수정
- 주석 수정
- 로그 메시지 수정

위 예외를 제외한 모든 변경은 `docs/plans/PLAN_<short_name>.md` 를 먼저 작성하고 사용자 승인 후 실행한다.

### 3.2 운영 규칙

계획서 작성 절차, 템플릿, Phase 구성, Done 조건, Commit Messages 규칙 등 상세 운영 규칙은 [docs/CLAUDE.md](docs/CLAUDE.md) 를 단일 SoT 로 따른다.

신규 계획서 템플릿: [docs/plans/\_template.md](docs/plans/_template.md)

**Plan 작성 위치**: 반드시 `docs/plans/` 하위에만 작성한다. `.claude/plans` / 루트 / 기타 위치는 금지.

**검증 체크리스트 의무**: 각 Phase 의 Validation 에 "사용자가 무엇을 보면 완료로 판단하는지" 를 구체적으로 명시한다. 본 프로젝트는 단위 테스트가 없으므로(§18) 사용자 실기 검증이 유일한 통과 기준이며, 그 기준을 모호하게 두면 Done 판정이 불가하다.

```
✗ 약한 기준: "정상 동작 확인"
✓ 강한 기준: "홈 탭 진입 시 portfolio 카드의 model_equity 가 $ 포맷으로 표시되고,
            오프라인 시 OfflineScreen 으로 즉시 전환된다"
```
...
- **더 단순한 대안이 보일 때** — push back. 사용자 요청을 그대로 수용하기 전에 단순화 가능성 제시
- **혼란스러울 때** — 멈춘다. 무엇이 혼란인지 명명한 뒤 질문. 추측으로 진행 금지
...
앱의 비즈니스 로직 구현 시 준수해야 하는 원칙. (데이터 불변성 / 명시적 검증 같은 일반 원칙은 §6 TypeScript 규칙과 §8 Zustand 규칙에 이미 녹아있으므로 본 섹션에서는 중복 기술하지 않는다.)
...
- 조용히 기본값을 반환하거나 건너뛰지 않는다 (`return null`, `continue`, `0` 대체 금지)
- 명시적으로 Error 를 발생시키거나 `console.error` + 안전 폴백으로 개발자가 즉시 인지할 수 있도록 한다
- 메시지에 `내부 불변조건 위반` 접두사와 위반된 변수/값을 포함한다

**TypeScript / React Native 적용 방식**:

앱 크래시가 사용자 경험을 해치므로 Python 의 `raise RuntimeError` 를 1:1 이식하지 않는다. §13.2 (Error Boundary 없음) 와 §1.4 (사용자 중심) 를 함께 고려하여, 다음 3가지 중 문맥에 맞는 방식을 선택한다:

**1) `__DEV__` 가드 + throw (기본 권장)** — 개발 빌드만 크래시, 릴리스는 로그 + 안전 폴백

```typescript
if (__DEV__) {
  throw new Error('[store] 내부 불변조건 위반: pendingOrders=null 인데 진입');
}
console.error('[store] 내부 불변조건 위반: pendingOrders=null');
return safeDefault;
```

**2) 타입 시스템으로 선차단** — discriminated union + exhaustive `never` 체크로 컴파일 타임에 막음

```typescript
const _exhaustive: never = state;
throw new Error(`내부 불변조건 위반: 알 수 없는 state=${_exhaustive}`);
```

**3) 치명적 경로만 throw** — 복구 불가능한 내부 모순(서버 계약 위반 등) 은 `throw new Error('내부 불변조건 위반: ...')` 로 즉시 중단. RN 기본 처리에 맡긴다(§13.2).

**구분 기준**:

- 입력 파라미터 검증 (외부에서 잘못된 값 전달 가능) → §13.4 사용자 표시 에러 규칙 적용
- 내부 로직 불변조건 (코드 흐름상 절대 도달 불가) → 위 3가지 방식 중 선택
...
- 외부 제약사항 (예: "RN 0.82+ 에서 New Architecture 강제, 끌 수 없음")
...
**주석 / 문서 / CLAUDE.md / README.md 공통 원칙.**

- **역할/책임 중심 설명**: "이 파일은 무엇을 담당하는가" 를 기록. "현재 어떤 값이 들어있는가" 는 기록하지 않는다.
- **구체적 수치 / 가변 정보 직접 기재 금지**: 버전, 상수 목록, 파일 목록 등 코드에서 파생 가능한 정보는 문서에 복제하지 않고 해당 파일을 참조한다.
  - 예: "버전은 `package.json` 참조", "상수 목록은 `src/utils/constants.ts` 참조"
- **리팩토링 내구성**: 리팩토링 후에도 쉽게 깨지지 않는 설명을 목표로 한다. 파일 경로 / 함수명 / 라인 번호에 과하게 의존하는 서술은 피한다.

---

## 10. Firebase SDK 사용 규칙
...
- `device_id` 는 설치 UUID. `AsyncStorage` 에 영구화(§8.4 예외). 근거 위치: `src/services/fcm.ts::getDeviceId`, `src/services/fcm.ts::requestNotificationPermission`
...
## 11. WebView + TradingView Charts 규칙
...
- **사용자에게 표시되는 에러는 §13.4 규칙 우선** — `console.error` 는 개발자 로그용, 사용자 메시지는 별도 한글 텍스트
...
> 내부 불변조건(로직상 도달 불가) 처리는 §5.1 불가능 조건 처리 를 따른다.
...
1. Claude Code 는 새 의존성 추가 시 **반드시 사용자에게 질문** (§4.2 형식)
...
- **`react-native` 와 `react` 는 정확한 버전 고정** (`^` 금지). 현재 고정된 버전은 `package.json` 참조.
- `@react-native/*` 모노레포 패키지들 (preset, config 등) 도 `react-native` 와 동일한 정확 버전으로 고정. RN 업그레이드 시 함께 움직임.
- 주요 외부 라이브러리 메이저 업그레이드 (예: Firebase v24 → v25) 는 사용자 승인 필요
- **RN 마이너 버전 업그레이드** 는 Upgrade Helper 기반 계획 + 사용자 승인 필요
...
- **앱에 GitHub 토큰 / GCS 자격증명 절대 없음**. GCS 정본 접근은 서버 전용.
...
| AsyncStorage / SQLite 로 RTDB 데이터·세션 캐싱 | 메모리 캐시만 (예외: 설치 UUID 1개, §8.4) |
| Redux / MobX / 기타 상태 라이브러리            | Zustand 통일                              |
| `/latest/*`, `/charts/*`, `/history/*` 쓰기    | 서버 전용                                 |
| inbox `processed` 필드 쓰기                    | 서버 전용 (읽기는 허용 — §10.3)           |
| GCS 정본 접근                                  | 서버 전용                                 |
...
- `applicationId` / `namespace` 는 `android/app/build.gradle` 참조
...
- 환경변수 사용 없음. 모든 상수는 `src/utils/constants.ts` 에 정의.
- 주요 카테고리: 식별자/접속(소유자 UID, RTDB URL, 패키지명, 앱 버전), 네트워크/타임아웃, 시간/날짜, UI 수치/레이아웃, 외부 라이브러리 버전, RTDB 경로, 자산, 기호 화이트리스트, 토스트 메시지.
- 전체 상수 목록 / 값 / 의미 주석은 [`src/utils/constants.ts`](src/utils/constants.ts) 를 단일 SoT 로 참조한다.

### 17.4 APP_VERSION 동기화
