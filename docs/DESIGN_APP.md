...
| 설치된 SDK Platform | Android 16 (API 36, RN 0.85 기본) |
...
| `targetSdkVersion` | **36** (Android 16) | RN 0.85.1 템플릿 기본값. Google Play 2025년 8월+ targetSdk 35 이상 요구 충족. |
| `compileSdkVersion` | **36** | targetSdkVersion 과 일치 |
...
- **탭 아이콘**: Feather-style SVG 인라인 — `react-native-svg` + `src/components/TabIcons.tsx` (`HomeIcon` / `ChartIcon` / `TradeIcon` / `SettingsIcon` 4종, 24×24 viewBox stroke). `AppNavigator.tsx` 에서 `tabBarIcon` 으로 연결, 활성/비활성 색은 `tabBarActiveTintColor` / `tabBarInactiveTintColor` 가 자동 전달.
...
- **사용**: 시스템 기본 폰트 (Android Roboto + 한글 자동 fallback Noto Sans CJK). `styles.xml` / JS StyleSheet 모두 `fontFamily` 미지정.
- **이력**: 초기 설계는 Pretendard Variable 전역 적용을 권장했으나, **RN 0.85 + Android 의 Text 측정-렌더 폰트 불일치** 로 영문 마지막 글자가 잘리는 이슈 발견 (2026-04-20). `styles.xml` 의 `android:fontFamily` 는 그리기에는 적용되지만 RN 의 측정 단계는 시스템 폰트 metrics 를 사용하여 Pretendard 의 더 넓은 advance width 와 mismatch → 측정 영역 밖으로 마지막 글자(D/O/Q/Y 등) 밀려남. Static OTF + `font-family.xml` weight 매핑까지 시도해도 동일 → 전역 적용 롤백.
- **관련 RN 이슈**: [#53017](https://github.com/facebook/react-native/issues/53017), [#53286](https://github.com/facebook/react-native/issues/53286), [#36992](https://github.com/facebook/react-native/issues/36992).
- **재도입 시 권장**: 모든 Text StyleSheet 에 `fontFamily` 직접 명시 (RN 측정도 해당 폰트 기준으로 강제), 또는 wrapper Text 컴포넌트 (`<PText>`) 로 일괄 적용. styles.xml Theme 단독 적용은 회피.
- **시스템 한글 폰트(Noto Sans CJK)** 가독성 충분하므로 MVP 범위에서 추가 폰트 도입 불필요.
...
- [x] Android SDK Platform 36 설치 (RN 0.85.1 `compileSdk` 기본값)
...
**검증**:
- [x] 첫 실행 시 로그인 화면 표시
- [x] 잘못된 이메일/비밀번호 → 에러 텍스트
- [x] 올바른 로그인 → 홈 탭 이동 + "QBT Live" 헤더
- [x] 4탭 전환 가능 (홈/차트/거래/설정)
- [x] 앱 재시작 → 자동 로그인
...
**검증**:
- [x] 홈 탭 진입 시 RTDB 데이터 로드 (loading 인디케이터)
- [x] 자산 현황 카드에 SSO/QLD/GLD/TLT + 현금 표시
- [x] 합계 금액 (`$10,424.50` 형식) 표시
- [x] 자산별 비중 (소수점 1자리 %, 예: `33.8%`) 표시
- [x] 배지 (`[보유]`/`[현금]`/`[매수대기]`/`[매도대기]`) 정상 표시
...
**검증**:
- [x] 경고 배지 (4일 초과) 동작 확인 (테스트로 execution_date 조작)
- [x] 리마인더 블록 표시/숨김 조건 정확
- [x] 시그널 블록 표시/숨김 조건 정확
- [x] MA 근접도 SPY/QQQ/GLD/TLT 순서 + 색상
- [x] Model 비교 접힘/펼침 애니메이션
- [x] 동기화 다이얼로그 → 동기화 → Toast → RTDB 확인
...
**검증**:
- [x] pending 있는 자산 선택 시 ⚡ 표시
- [x] 매도 수량이 보유 주수 초과 시 에러
- [x] 체결 저장 → Toast + 폼 초기화 + RTDB `/fills/inbox/` 확인
- [x] 보정 저장 → Toast + 폼 초기화 + RTDB `/balance_adjust/inbox/` 확인
- [x] 시그널 스킵 → Toast + RTDB `/fill_dismiss/inbox/` 확인
- [x] 히스토리 필터 (전체/체결/보정/신호) 정상 동작
- [x] 히스토리 행 색상 (초록/빨강/파랑/노랑) + 배지 ([시스템]/[개인])
...
**검증**:
- [x] 주가 차트: 4자산 전환 + 종가/EMA-200/상단/하단 밴드 라인
- [x] 마커 4종 (▲매수시그널/▼매도시그널/●내매수/●내매도) 표시
- [x] Equity 차트: Model / Actual 2라인
- [x] 핀치 줌 / 드래그 동작
- [x] 좌측 끝 도달 → archive 자동 로드 + dedupe
- [x] 기간 선택 버튼 없음 확인
...
- [x] 시스템 기본 폰트 (Roboto + Noto Sans CJK) 정상 표시 — Pretendard 전역 적용 시도 후 영문 잘림으로 롤백 (§11.2)
- [ ] APK 빌드 성공
...
- [x] 4탭 모두 정상 동작
- [x] 로그인 → 자동 로그인 → 로그아웃 전 플로우
- [x] RTDB 읽기/쓰기 모두 정상 (5개 경로)
- [x] 토스트 4종 모두 표시 (체결/보정/스킵/동기화)
- [ ] 배지 시스템 전 규칙 동작 (§11.3)
- [x] 색상 상수 100% 사용 (하드코딩 hex 없음)
- [x] 이모지 0개 (§17.3)
- [x] 시스템 기본 폰트 (Roboto + Noto Sans CJK) 적용 — Pretendard 전역 적용 시도 후 영문 잘림 이슈로 롤백 (§11.2)
- [x] 한국어 금액 포맷 정상 (§16.1)
- [x] 차트 마커 4종 + 범례 표시
- [x] 차트 archive 점진 로딩 + dedupe
- [x] Pull-to-refresh 4탭 모두
- [ ] 오프라인 화면 차단 + 복구
- [ ] FCM 토큰 등록 + 알림 탭 → 홈
- [x] `google-services.json` Git 제외
- [ ] APK 빌드 성공 + 실기기 설치
- [x] 타입 에러 0개 (`tsc --noEmit`)
