...
- [ ] 합계 금액 형식 `$10,424.50` 정상 (`formatUSD`)
- [ ] 자산별 비중 `(actual_shares × close) / actual_equity × 100` 기반 소수점 1자리 정상 (예: `33.8%`)
- [ ] 현금 비중 `shared_cash_actual / actual_equity × 100` 정상
...
- [ ] `formatKRW` 호출 0곳 (`grep -r "formatKRW" src/` 결과 비어있음 — 통화 USD 통일 반영)
- [ ] 하드코딩 hex 0곳
