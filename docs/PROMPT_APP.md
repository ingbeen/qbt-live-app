...
- [x] Android SDK Platform 35 설치 (`compileSdk 35` 기본값)
...
- `docs/DESIGN_APP.md §1.3` — Android 빌드 설정 (compileSdk 35, New Arch ON)
...
- [ ] `package.json` 에 §1.2 의 모든 의존성 반영
- [ ] `applicationId`, `namespace` 가 `com.ingbeen.qbtlive`
- [ ] Display name 이 "QBT Live"
- [ ] `tsconfig.json` 에 strict 모드 활성
- [ ] 에뮬레이터에 "QBT Live" 이름으로 앱 설치
- [ ] 앱 실행 시 Hello World 텍스트 표시 (다크 배경)
- [ ] `adb logcat` 에서 `[Firebase] initialized, persistence OFF` 로그 확인
- [ ] Firebase native module 관련 에러 0
- [ ] Hot reload 동작
- [ ] `tsc --noEmit` 에러 0
- [ ] debug SHA-1 Firebase Console 에 등록 완료
- [ ] `google-services.json` 이 SHA-1 포함된 최신 버전으로 교체됨
- [ ] `.gitignore` 가 `google-services.json` 을 여전히 제외 (`git status` 에 표시 안 됨)
- [ ] `[Phase 1]` 커밋이 GitHub 에 push 됨
