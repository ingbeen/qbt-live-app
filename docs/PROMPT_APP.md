...
> **관련 문서**:
> - `CLAUDE.md` — 앱 코딩 규칙 + 아키텍처 + 배포 체크리스트 (단일 SoT)
> - `docs/DESIGN_QBT_LIVE_FINAL.md` — 서버↔앱 RTDB 계약 (서버 SoT)
>
> **참고**: 본 문서 본문의 `docs/DESIGN_APP.md §X` 참조는 **역사적 기록**이다. 해당 설계서는 2026-04-21 CLAUDE.md 로 이관 후 삭제되었다. 현재 기준 참조 위치:
> - 아키텍처 / 데이터 흐름 → CLAUDE.md §18
> - 오프라인 / 최신성 → CLAUDE.md §19
> - RTDB 보안 규칙 → CLAUDE.md §20
> - FCM 알림 정책 / Android 13+ 권한 → CLAUDE.md §6.5
> - 금지 사항 / 비목표 → CLAUDE.md §12
> - 타입 / Store / 유틸 / 화면 구현 → `src/` 코드 자체가 SoT
> - Firebase 운영 절차 / 디버깅 / 배포 체크리스트 → 사람용 운영 정보로 문서에서 제거됨
...
- Android 13+ 에서 알림 권한은 **런타임 요청** 필요. `PermissionsAndroid.request(POST_NOTIFICATIONS)` 호출 시점 = **앱 시작 시** (로그인 전, `App.tsx` 첫 useEffect). 토큰 등록과 관심사 분리: `ensureFcmToken` 은 로그인 후 `PermissionsAndroid.check` 로 상태만 확인.
...
- **`@react-native-firebase/messaging` modular API**: v24 는 database/auth 와 동일한 tree-shakable 패턴 (`getMessaging(getApp())` + top-level 함수 `getToken(messaging)`, `onMessage(messaging, cb)`, `onTokenRefresh(messaging, cb)`, `onNotificationOpenedApp(messaging, cb)`, `getInitialNotification(messaging)`). namespaced `messaging()` 금지. iOS 전용 `requestPermission(messaging)` / `AuthorizationStatus` 는 Android 전용 프로젝트라 사용 안 함 — POST_NOTIFICATIONS 권한은 `PermissionsAndroid` 로 요청.
...
- **Android 13+ 런타임 권한**: `@react-native-firebase/messaging` 의 `requestPermission` 은 iOS 전용이라 Android 에서 다이얼로그가 뜨지 않고 즉시 AUTHORIZED 반환. POST_NOTIFICATIONS runtime 권한은 **`PermissionsAndroid.request(POST_NOTIFICATIONS)` 로 명시 요청** 필요 (App.tsx 첫 useEffect). AndroidManifest 에 `<uses-permission android:name="android.permission.POST_NOTIFICATIONS" />` 선언도 필수. 실제 권한 상태 확인은 `PermissionsAndroid.check` 사용.

#### 테스트 가능 환경 검증 대기 항목

- [x] 에뮬레이터/실기기 `npm run android -- --no-packager` 설치 후 로그인 → logcat 에 `[fcm] token registered` 표시 확인
- [x] Firebase Console → Realtime Database `/device_tokens/` 경로에 `{uuid}` 키 + 토큰 문자열 저장 확인
- [x] Firebase Console → Cloud Messaging 테스트 발송 (title/body 입력) → 앱 백그라운드 상태에서 시스템 트레이 알림 수신 확인
- [x] 알림 트레이 탭 → 앱 열림 + 홈 탭 자동 선택 확인 (현재 탭이 설정/차트/거래 일 때 홈으로 전환되는지)
- [x] 앱 종료 상태에서 알림 탭 → 앱 cold start + `getInitialNotification` 감지 → 홈 탭 진입 확인
- [x] 포그라운드 상태에서 FCM 수신 시 인앱 토스트/다이얼로그 **미표시** 확인 (무시 정책 §6.5)
- [x] 로그아웃 버튼 탭 → Auth 해제 → LoginScreen 자동 전환 + 재로그인 시 새 FCM 등록 흐름 재시작
- [x] Android 13+ 기기에서 최초 실행 시 알림 권한 프롬프트 표시 + 거부 시 `[fcm] notification permission denied` warn 로그
...
## Phase 8: 오프라인 / 에러 처리 ✅ **코드 완료 (2026-04-20) / 런타임 검증 대기**
...
- [x] `tsc --noEmit` 에러 0
- [x] 금지 패턴 grep 6종 0건
- [x] `setupNetworkListener` 구독 + `isOnline` 상태 동기화 (code)
- [x] `isOnline === false` → OfflineScreen 전체 차단 렌더 분기 (code)
- [x] `AppState.active` 복귀 시 `clearAll()` + `refreshHome()` (code, user 존재 시만)
- [x] `clearAll` 리팩토링 검증: user / isOnline / deviceId / fcmRegistered 유지 (Phase 7 선행)
- [x] `ErrorState` 컴포넌트 구현 + HomeScreen 의 portfolio===null 상태에 통합
- [x] 10초 타임아웃 기존 `withTimeout` 재사용 (rtdb.ts, 기존 구현)
- [x] 권한 오류 한글 메시지: 기존 `toUserMessage` 에서 `PERMISSION_DENIED` → "권한이 없습니다..." 변환 (기존 구현)
- [x] 에러 메시지 전부 한글, stack trace 미노출
- [x] ACCESS_NETWORK_STATE 권한 선언
- [x] `UpdateStatusBadge` 기반 execution_date 경고 배지 (Phase 4 선행 기능 재확인)
- [~] 에뮬레이터 airplane mode ON → 즉시 OfflineScreen 표시
- [~] Airplane mode OFF → 자동 복구 (NetInfo 리스너 setOnline(true))
- [~] RTDB 타임아웃 유도 → ErrorState + 재시도 동작 확인
- [~] 앱 백그라운드 → 포그라운드 전환 → logcat 에 RTDB 재요청 로그 확인
- [~] `portfolio.execution_date` 5일 이상 경과 시 홈 상단 [경고] 배지 노랑 (실기 확인)
...
Phase 8 은 앱의 **연결성 / 최신성 보장 계층** 을 완성. NetInfo 리스너가 네트워크 단절을 감지하는 순간 전체 화면을 `OfflineScreen` 으로 차단하고, 포그라운드 복귀 시 캐시를 무효화하여 stale 데이터로 오판하는 것을 방지한다. 전체 로드 실패 상황은 `ErrorState` 공통 컴포넌트로 재시도 UI 를 제공.

**세션 제약**: 실기기/에뮬레이터 접근 불가. 코드 작성 + `tsc --noEmit` + grep 6종 검증까지 수행. airplane mode 토글, AppState 전환 logcat 등 런타임 검증은 `[~]` 로 이관.

#### 신규 의존성

- **없음**. `@react-native-community/netinfo@^12.0.1` 은 Phase 1 에서 이미 설치됨. 네이티브 모듈 추가 없음 → Gradle clean 불필요. NetInfo v12 는 RN 0.85 New Architecture (Fabric) 호환.

#### 계획 대비 추가된 수정

1. **`HomeScreen` 인라인 에러 UI 를 `ErrorState` 로 추출 + 교체**: Phase 4 에서 HomeScreen 이 이미 `portfolio===null` + `lastError` 시 자체 "다시 시도" UI 를 가지고 있었음 (`centerContainer` + `errorText`/`emptyText` + `retryButton` 스타일). Phase 8 에서 같은 패턴을 다른 탭 (거래/차트) 에서도 재사용 가능하도록 `src/components/ErrorState.tsx` 로 추출. HomeScreen 의 관련 스타일 5종 (`errorText`, `emptyText`, `retryButton`, `retryText`) 제거. `lastError ?? '데이터가 없습니다.'` 로 메시지 통합 (빈 상태와 오류 상태를 한 UI 로 표시 — YAGNI).

2. **`OfflineScreen` 에 재시도 로딩 인디케이터**: 설계서 §12.2 는 [다시 시도] 버튼만 명시. NetInfo.refresh() 가 수 초 걸릴 수 있어 사용자에게 피드백 필요 → `retrying` state + `ActivityIndicator` 표시. 중복 탭 방지 `disabled` 가드도 포함.

3. **`SYMBOLS.WARN` 큰 글씨 아이콘**: 설계서 §12.2 는 "SVG disconnect 아이콘" 을 언급하나 신규 라이브러리 도입 피하기 위해 (AskUserQuestion 승인) 기존 `SYMBOLS.WARN` (`⚠` U+26A0) 을 `fontSize: 64` + `COLORS.yellow` 로 렌더. §17.4 허용 기호 범위 + `lineHeight: 72` 로 일부 Android 기기에서 클리핑 방지.

4. **`AppState` 리스너를 최상위 useEffect 에 배치 (user 무관)**: 설계서 §12.4 는 AppState 콜백 내부에서 `useStore.getState().user` 체크로 로그인 상태 판단. 즉 AppState 리스너 자체는 user 여부에 독립적으로 등록. 로그아웃 상태에서도 앱 resume 이벤트를 받지만 refreshHome 은 호출하지 않음.

5. **`isOnline` gate 를 렌더 트리 최상위에 배치**: `NavigationContainer` 앞에 early return. Login 화면이든 탭 화면이든 모두 오프라인이면 차단. SafeAreaProvider + GestureHandlerRootView 는 내부적으로 계속 감싸 (StatusBar / gesture handler 정상 동작).

6. **`refreshNetworkState` 라는 래퍼**: `OfflineScreen` 이 `NetInfo.refresh()` 를 직접 호출하지 않고 `src/services/network.ts` 의 래퍼 경유. 컴포넌트 레이어가 `@react-native-community/netinfo` 에 직접 의존하지 않도록 (CLAUDE.md §17.3 모듈 독립성).

7. **`firebase.ts` / `rtdb.ts` 추가 변경 없음**: 설계서 §12.4 는 "RTDB persistence OFF 유지" 를 Phase 6 이전에 이미 적용 (`firebase.ts` 의 `setPersistenceEnabled(db, false)`). Phase 8 에서 재확인만. `withTimeout` 10초 도 기존 `rtdb.ts` 에 이미 존재.

8. **`ErrorState` 컴포넌트 재사용 범위 제한**: Phase 8 에서는 HomeScreen 에만 적용. TradeScreen/ChartScreen 은 각각 고유 에러 UX (거래 탭은 Toast + 입력 필드 disable, 차트 탭은 WebView 범위) 가 있어 일괄 적용이 혼란 유발 가능 → YAGNI. 필요 시 향후 Phase 에서 추가.

#### 실행 중 확인된 팁

- **`NetInfo.addEventListener` 초기 호출 동작**: 리스너 등록 직후 현재 네트워크 상태를 1회 즉시 콜백으로 발송. 따라서 `useStore.isOnline` 기본값 true 는 리스너 등록 직후 실제 상태로 자동 정정. 별도 `NetInfo.fetch()` 초기 호출 불필요.
- **Android `isInternetReachable` null 케이스**: 네트워크 전환 순간이나 VPN 연결 시 일시적으로 null. `!== false` 패턴이 true/null 모두 온라인으로 판단해 깜빡임 방지.
- **`AppState.active` 가 cold start 시 트리거되지 않음**: 앱이 이미 active 로 시작하므로 initial callback 없음. 따라서 App.tsx 의 `initFirebase` + `subscribeAuthState` 와 HomeScreen 의 `useEffect(refreshHome)` 경로가 초기 데이터 로드 담당. AppState 리스너는 background → foreground **전환 시에만** 재로드.
- **`clearAll` 리팩토링의 의도치 않은 이점**: Phase 7 에서 분리한 (user/isOnline/deviceId/fcmRegistered 유지) 구조 덕분에 AppState 복귀 시 "로그인 유지 + 네트워크 상태 유지 + FCM 토큰 유지 + 캐시만 무효화" 가 자연스럽게 달성됨. 추가 플래그/액션 없이 단일 clearAll 재사용.
- **`OfflineScreen` 상태 전환**: isOnline=false 차단 중에는 NavigationContainer 가 마운트되지 않음. 복귀 시 NavigationContainer 가 다시 마운트되며 기본 탭(홈)으로 돌아옴. 사용자가 차트/거래 탭을 보던 중 네트워크 단절이 발생하면 복귀 후 홈으로 리셋되는 동작 — 설계서 §12.2 와 일치.
- **`ErrorState` 와 홈 탭 `errorBanner` 공존**: 홈 탭은 `portfolio != null` 상태에서도 부분 실패 시 화면 상단 `errorBanner` (빨강 1줄) 표시 유지. 전체 실패(portfolio null) 시에만 `ErrorState` 로 전환. 부분/전체 구분이 유지됨.

#### 테스트 가능 환경 검증 대기 항목

- [ ] 에뮬레이터 "Cellular data off + WiFi off" 또는 airplane mode ON → OfflineScreen 즉시 표시 확인
- [ ] 네트워크 복귀 → OfflineScreen 사라지고 이전 화면 (로그인 or 홈) 복귀 확인
- [ ] NetInfo 리스너 초기 1회 자동 콜백 발동 확인 (logcat 에서 setOnline 로그)
- [ ] 앱 백그라운드 → 포그라운드 전환 시 logcat 에 `[rtdb]` / `[store]` refreshHome 재요청 로그 확인
- [ ] RTDB 타임아웃 유도 (예: Firebase Console 에서 Rules 를 일시적으로 deny 로 설정) → ErrorState + [다시 시도] 동작 확인 → 원복
- [ ] PERMISSION_DENIED 오류 유도 → "권한이 없습니다. OWNER_UID 설정을 확인하세요." 메시지 표시 확인
- [ ] `portfolio.execution_date` 가 오늘 기준 5일 이상 전인 상태 구성 (또는 로컬 오늘 날짜 조정) → 홈 상단 [경고] 배지 노랑 표시 확인
- [ ] 에러 상태에서 stack trace / TypeError 등 영문 예외명 노출 없음 확인
- **Firebase Console 작업 필요 여부**: 없음. 권한 오류 유도 테스트 외 Console 설정 변경 불필요.
- **google-services.json 재다운로드 필요 여부**: 없음. 네이티브 설정 변경 없음.

#### Phase 9 로 이관된 후속 작업

- **최종 마감 (Phase 9 주제)**: 전체 grep 6종 검증, Pretendard 폰트 설정(`react-native.config.js`), versionCode/versionName 1.0.0 확정, package.json version 1.0.0.
- **`ErrorState` 를 TradeScreen/ChartScreen 으로 확장**: 필요성 판단은 런타임 검증 후. 실기에서 Trade/Chart 탭의 로드 실패 상황이 실제로 불편한지 확인 후 추가.
- **RTDB persistence 관련 재검증**: 현재 OFF 유지 상태에서 오프라인 복귀 후 첫 쿼리 성공/실패 패턴 logcat 확인 (CLAUDE.md §6.1 OFF 고정 방침 유지 전제).

---

## Phase 9: 마무리 ◐ **정적 부분 코드 완료 (2026-04-20) / 폰트·아이콘·빌드·실기 검증 대기**
...
마킹 범례: `[x]` = 정적 검증 완료 / `[~]` = 코드/설정 완료, 런타임/빌드 검증 이관 / `[ ]` = 미구현

- [x] 하드코딩 hex 색상 0곳 (COLORS 상수 + chartHtml.ts WebView 예외만)
- [x] `any` 타입 0곳 (명시적 `: any` / `as any` 0건)
- [x] `onValue` 0곳
- [x] `AsyncStorage` / `SQLite` 0곳
- [x] 이모지 0개 (U+1F300~U+1FAFF 범위)
- [x] `formatKRW` 0곳 (통화 기준 USD 준수)
- [x] `versionName "1.0.0"` 확정 (`android/app/app.build.gradle`)
- [x] `package.json` version 1.0.0
- [x] `react-native.config.js` 신규 — `project.ios: null` + `assets: ['./assets/fonts/']`
- [x] `tsc --noEmit` 에러 0
- [~] Pretendard 폰트 `.ttf` 파일 `android/app/src/main/assets/fonts/` 배치
- [~] `npx react-native-asset` 실행 → Android assets 링크
- [~] 전역 `Text` 스타일 `fontFamily: 'Pretendard'` 적용 (폰트 파일 확보 후)
- [~] 앱 아이콘 교체 (`android/app/src/main/res/mipmap-*`)
- [~] 서명 키스토어 생성 + `android/gradle.properties` 비밀번호 설정
- [~] `./gradlew assembleRelease` APK 빌드 성공 + 50MB 이하 확인
- [~] 실기기 설치 + 로그인 + 홈 데이터 로드 성공
- [~] 실기기 FCM 수신 + 알림 탭 홈 이동 최종 확인
- [~] `docs/DESIGN_APP.md §23` 최종 육안 체크리스트 통과
...
Phase 9 중 **정적 부분만** 본 세션에서 처리. 실기기/에뮬레이터 접근 불가 환경 제약으로 폰트 `.ttf` 배치, 앱 아이콘 생성, APK 빌드, 실기 설치는 후속 세션 이관.

**이 세션에서 수행한 작업**:
1. 전체 소스 grep 6종 최종 검증 → 0건 (`onValue` / `formatKRW` / 하드코딩 hex / `any` / 이모지 / `AsyncStorage`·`SQLite`)
2. `npx tsc --noEmit` → EXIT=0
3. `react-native.config.js` 신규 — 폰트 assets 경로 등록 스캐폴딩
4. `android/app/build.gradle` versionName `"1.0"` → `"1.0.0"` 확정
5. `package.json` version `"0.0.1"` → `"1.0.0"` (SettingsScreen 의 `APP_VERSION` 상수와 일치)

**AskUserQuestion 으로 확정된 정책**:
- **OfflineScreen 아이콘**: `SYMBOLS.WARN` 사용 (Phase 8 에서 적용 완료). react-native-svg 도입 불필요.
- **Pretendard 폰트**: `.ttf` 파일 없음 → `react-native.config.js` 만 작성. 전역 Text 스타일 적용은 폰트 확보 후 이관 (파일 없이 적용하면 시스템 폰트로 폴백).
- **앱 아이콘**: SVG 소스 없음 → 기본 Android 런처 아이콘 유지 + 테스트 가능 환경 이관.

#### 신규 의존성

- **없음**. `react-native-asset` CLI 는 필요 시 `npx` 로 일회성 호출 (devDependency 로 고정하지 않음).

#### 계획 대비 추가된 수정

1. **`react-native.config.js` 에 `project.ios: null` 포함**: CLAUDE.md §0 의 "Android 전용, iOS 지원 안 함" 방침에 맞춰 iOS autolinking 스킵. 플래그 없이도 이 프로젝트는 iOS 빌드를 안 하지만, 명시적 `ios: null` 로 의도 고정 (미래 `npx react-native-asset` 이 ios 폴더를 터치하지 않도록).

2. **전역 `Text` fontFamily 적용 보류**: 설계서 §11.2 는 "전역 Text 에 fontFamily: 'Pretendard'" 지시. 그러나 `Text.defaultProps` 는 RN 0.72+ 에서 deprecated (New Architecture 경고). Custom `AppText` 컴포넌트로 전 화면 교체는 YAGNI 위반(모든 Text 리팩토링). 결론: **폰트 파일 확보 후** `android/app/src/main/res/values/styles.xml` 의 `AppTheme` 에 `android:fontFamily="@font/pretendard"` 추가하거나 RN 측 SafeAreaProvider 래퍼 스타일 주입 방식 검토. 이 세션에선 설정 파일(`react-native.config.js`)만 생성.

3. **`package.json version` 을 `0.0.1` → `1.0.0` 으로 건너뜀**: 마이너 릴리스 히스토리 없이 1.0.0 직행 — MVP 완성 지점이 첫 공식 버전이므로 적절.

4. **`versionCode` 는 1 유지**: 기존 값 그대로. Play Store 제출 시점부터 증가. 현재는 APK 사이드로드 MVP 이므로 1 에서 멈춤.

5. **전체 소스 grep 기준**: 계획의 "6종 grep" 은 `onValue` / `formatKRW` / 하드코딩 hex / `any` / 이모지 / `AsyncStorage`·`SQLite`. `colors.ts` 와 `chartHtml.ts` 의 hex 는 각각 COLORS 정의 파일 / WebView HTML 내부 예외(§7) 로 허용. Phase 9 최종 검증에서도 두 파일만 매치 → 규칙 준수.

#### 실행 중 확인된 팁

- **`react-native.config.js` 의 `assets` 경로**: 프로젝트 루트 기준 상대경로. `'./assets/fonts/'` 는 RN CLI 가 해당 폴더 내 `.ttf`/`.otf` 파일을 Android `app/src/main/assets/fonts/` 에 자동 복사. 폴더가 비어있거나 없으면 아무 동작 안 함 (에러 아님) → 현재 설정이 런타임에 영향 없음.
- **`npx react-native-asset` vs `npx @react-native-community/cli link-assets`**: RN 0.85 (v23+ `@react-native-community/cli`) 에서는 autolinking 이 기본이므로 `link-assets` 는 폰트 등 정적 리소스에만 필요. `react-native-asset` 는 설정 기반 폰트 배포 자동화 패키지 (번들된 CLI 아님). 이 프로젝트에선 `react-native-asset` (커뮤니티 패키지) 설치 없이 `npx react-native-asset` 으로 일회성 호출 가능.
- **versionCode 증가 정책**: Play Store 는 항상 증가 요구. APK 사이드로드 MVP 는 재설치 시 덮어쓰기 되므로 1 유지 가능. 사용자가 Play Store 업로드 시점 이후부터 매 릴리스 +1.
- **IDE TS 서버 캐시 경고 재발 가능성**: `react-native.config.js` 추가 후 IDE 가 새 파일을 인덱싱하면서 일시적 경고 표시 가능. `tsc --noEmit` CLI 는 .js 파일 무시(`tsconfig.json include: ["**/*.ts", "**/*.tsx"]`) 이라 영향 없음.

#### 테스트 가능 환경 검증 대기 항목

- [ ] Pretendard-Variable.ttf (또는 Pretendard-Regular/Bold 페어) 파일을 `./assets/fonts/` 에 배치 → `npx react-native-asset` 실행 → `android/app/src/main/assets/fonts/` 에 복사되었는지 확인
- [ ] 전역 Text fontFamily 적용 결정 (styles.xml vs custom AppText 컴포넌트) 후 적용 → 에뮬레이터 렌더에서 Pretendard 반영 확인
- [ ] 앱 아이콘 SVG/PNG 소스 준비 → Android Studio Image Asset Studio (Launcher Icon adaptive) → `mipmap-*` 자동 배치 → 런처 아이콘 교체 확인
- [ ] 서명 키스토어 생성: `keytool -genkey -v -keystore qbt-live-release.keystore -alias qbtlive -keyalg RSA -keysize 2048 -validity 10000` → `android/gradle.properties` 비밀번호 등록 (커밋 금지)
- [ ] `cd android && ./gradlew assembleRelease` → `android/app/build/outputs/apk/release/app-release.apk` 생성 확인 + 크기 50MB 이하
- [ ] Release variant SHA-1 `keytool -list -v -keystore qbt-live-release.keystore -alias qbtlive` → Firebase Console 에 추가 등록
- [ ] `adb install -r app-release.apk` → 실기기/에뮬레이터 설치 성공
- [ ] 설치된 앱에서 로그인 → 홈 탭 데이터 로드 → 차트 탭 렌더 → 거래 탭 폼 동작 → 설정 탭 6행 표시 확인
- [ ] Firebase Console Cloud Messaging → 실기기로 테스트 발송 → 백그라운드 알림 수신 → 알림 탭 → 홈 탭 진입 확인
- [ ] 에뮬레이터 airplane mode 토글 → OfflineScreen 차단 / 복귀 확인 (Phase 8 DoD)
- [ ] `docs/DESIGN_APP.md §23` 최종 체크리스트 (색상 / 배지 / Pretendard / 이모지 / 4탭 / 토스트 / 푸시 / 오프라인) 전부 육안 확인
- **Firebase Console 작업 필요 여부**: release SHA-1 등록 필요. debug SHA-1 은 Phase 1 에서 등록 완료.
- **google-services.json 재다운로드 필요 여부**: release SHA-1 추가 등록 후 최신본 다운로드 권장 (로컬 파일 교체 — git 커밋 금지).
...
5. 실기기 설치 + 로그인/차트/거래/설정 E2E 검증
6. FCM 수신 + 알림 탭 홈 이동 실기 검증
7. Airplane mode 토글 + AppState 복귀 실기 검증
8. `docs/DESIGN_APP.md §23` 최종 체크리스트 육안 통과

본 세션에서는 **정적 검증 및 설정 파일 작성까지 완료**. Git commit 은 사용자가 직접 수행.
