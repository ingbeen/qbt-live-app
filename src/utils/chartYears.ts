// 다음으로 로드가 필요한 연도를 반환한다. 없으면 null.
// 현재 가장 오래된 연도 슬라이스의 첫 날짜와 이미 로드된 연도 목록을 기준으로
...
//      아니며 availableYears 에 포함 → firstYear
//      (해당 연도 앞쪽 구간을 채우기 위함)
//   3) 그 외 Math.min(firstYear, ...loadedYears) - 1 이 availableYears 에
//      포함 → 해당 전년도
//   4) 어느 조건도 해당 안 되면 null (더 로드할 데이터 없음)
export const computeNextYear = (
  ...
  if (
    !coversFullYear &&
    !firstYearLoaded &&
    availableYears.includes(firstYear)
  ) {
    return firstYear;
  }
...
export const computeInitialYears = (
  ...
  for (let y = targetYear; y <= lastYear; y += 1) {
    if (availableSet.has(y)) result.push(y);
  }
  return result;
};
